...
| CN-1 | Token continuity is convention-enforced (wallet/indexer) by default; **Script-enforced when the OD-3 covenant is used** (`internal/covenant`, built). | **RESOLVED** (OD-3 covenant built) | Without the covenant a hostile spender could strip the token (detected, not prevented); **with** the covenant a strip/mutation is **prevented** in Script. | OD-3 covenant (`docs/02` §2.6, `docs/08` §8.4) |
...
| **OD-3** | Script-enforced token continuity (`OP_PUSH_TX` covenant) in Stage 1, or defer? | **RESOLVED — pulled in (Stage 2)**; built in `internal/covenant`, interpreter-verified (`docs/08` §8.4). Continuity is now **Script-enforced**. | Pull in → continuity is **Script-enforced** (prevented, not just detected), plain Script, still no `OP_RETURN` (BSVM/Rúnar allowed, not required); adds covenant complexity and per-tx overhead. Defer → simpler Stage 1, continuity is a detected-not-prevented property. (`docs/02` §2.6) |
